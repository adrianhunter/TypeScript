...
// var zigparserPool = sync.Pool{
// 	New: func() any {
// 		return newParser()
// 	},
// }

// func getZigParser() *ZigParser {
// 	return parserPool.Get().(*ZigParser)
// }

// func putZigParser(p *ZigParser) {
// 	*p = ZigParser{scanner: p.scanner, setParentFromContext: p.setParentFromContext}
// 	parserPool.Put(p)
// }

// func ParseZigSourceFile(opts ast.SourceFileParseOptions, sourceText string, scriptKind core.ScriptKind) *ast.SourceFile {
// 	p := getZigParser()
// 	defer putZigParser(p)
...
// }
// func (p *ZigParser) initializeClosures() {
...
// func (p *ZigParser) isJavaScript() bool {
// 	return p.scriptKind == core.ScriptKindJS || p.scriptKind == core.ScriptKindJSX
// }

// func (p *ZigParser) parseJSONText() *ast.SourceFile {
// 	pos := p.nodePos()
...
// 				if p.lookAhead(func(p *ZigParser) bool {
...
// 				if p.lookAhead(func(p *ZigParser) bool { return p.nextToken() != ast.KindColonToken }) {
...
// func (p *ZigParser) validateJsonValue(sourceFile *ast.SourceFile, valueExpression *ast.Expression) {
...
// func (p *ZigParser) validateJsonObjectLiteral(sourceFile *ast.SourceFile, node *ast.ObjectLiteralExpression) {
...
// func (p *ZigParser) initializeState(opts ast.SourceFileParseOptions, sourceText string, scriptKind core.ScriptKind) {
...
// func (p *ZigParser) scanError(message *diagnostics.Message, pos int, length int, args ...any) {
// 	p.parseErrorAtRange(core.NewTextRange(pos, pos+length), message, args...)
// }

// func (p *ZigParser) parseErrorAt(pos int, end int, message *diagnostics.Message, args ...any) *ast.Diagnostic {
// 	return p.parseErrorAtRange(core.NewTextRange(pos, end), message, args...)
// }

// func (p *ZigParser) parseErrorAtCurrentToken(message *diagnostics.Message, args ...any) *ast.Diagnostic {
// 	return p.parseErrorAtRange(p.scanner.TokenRange(), message, args...)
// }

// func (p *ZigParser) parseErrorAtRange(loc core.TextRange, message *diagnostics.Message, args ...any) *ast.Diagnostic {
...
// func (p *ZigParser) mark() ParserState {
// 	return ParserState{
...
// func (p *ZigParser) rewind(state ParserState) {
...
// func (p *ZigParser) lookAhead(callback func(p *ZigParser) bool) bool {
// 	state := p.mark()
// 	result := callback(p)
// 	p.rewind(state)
// 	return result
...
// func (p *ZigParser) nextToken() ast.Kind {
...
// func (p *ZigParser) nextTokenWithoutCheck() ast.Kind {
// 	p.token = p.scanner.Scan()
// 	return p.token
// }

// func (p *ZigParser) nextTokenJSDoc() ast.Kind {
// 	p.token = p.scanner.ScanJSDocToken()
// 	return p.token
// }

// func (p *ZigParser) nextJSDocCommentTextToken(inBackticks bool) ast.Kind {
// 	p.token = p.scanner.ScanJSDocCommentTextToken(inBackticks)
// 	return p.token
// }

// func (p *ZigParser) nodePos() int {
// 	return p.scanner.TokenFullStart()
// }

// func (p *ZigParser) hasPrecedingLineBreak() bool {
// 	return p.scanner.HasPrecedingLineBreak()
// }

// func (p *ZigParser) jsdocScannerInfo() jsdocScannerInfo {
...
// func (p *ZigParser) parseSourceFileWorker() *ast.SourceFile {
// 	isDeclarationFile := tspath.IsDeclarationFileName(p.opts.FileName)
// 	if isDeclarationFile {
// 		p.contextFlags |= ast.NodeFlagsAmbient
// 	}
// 	pos := p.nodePos()
// 	statements := p.parseListIndex(PCSourceElements, (*ZigParser).parseToplevelStatement)
// 	end := p.nodePos()
...
// func (p *ZigParser) finishSourceFile(result *ast.SourceFile, isDeclarationFile bool) {
...
// func (p *ZigParser) createJSDocCache() map[*ast.Node][]*ast.Node {
...
// func (p *ZigParser) parseToplevelStatement(i int) *ast.Node {
...
// func (p *ZigParser) reparseTopLevelAwait(sourceFile *ast.SourceFile) *ast.Node {
...
// func (p *ZigParser) parseListIndex(kind ParsingContext, parseElement func(p *ZigParser, index int) *ast.Node) []*ast.Node {
...
// func (p *ZigParser) parseList(kind ParsingContext, parseElement func(p *ZigParser) *ast.Node) *ast.NodeList {
// 	pos := p.nodePos()
// 	nodes := p.parseListIndex(kind, func(p *ZigParser, _ int) *ast.Node { return parseElement(p) })
...
// func (p *ZigParser) parseDelimitedList(kind ParsingContext, parseElement func(p *ZigParser) *ast.Node) *ast.NodeList {
// 	pos := p.nodePos()
// 	saveParsingContexts := p.parsingContexts
// 	p.parsingContexts |= 1 << kind
// 	list := make([]*ast.Node, 0, 16)
...
// func (p *ZigParser) parseBracketedList(kind ParsingContext, parseElement func(p *ZigParser) *ast.Node, opening ast.Kind, closing ast.Kind) *ast.NodeList {
...
// func (p *ZigParser) parseEmptyNodeList() *ast.NodeList {
// 	return p.newNodeList(core.NewTextRange(p.nodePos(), p.nodePos()), nil)
// }

// func (p *ZigParser) createMissingList() *ast.NodeList {
...
// func (p *ZigParser) abortParsingListOrMoveToNextToken(kind ParsingContext) bool {
...
// func (p *ZigParser) isInSomeParsingContext() bool {
...
// func (p *ZigParser) parsingContextErrors(context ParsingContext) {
// 	switch context {
...
// func (p *ZigParser) isListElement(parsingContext ParsingContext, inErrorRecovery bool) bool {
...
// 	case PCTypeMembers:
// 		return p.lookAhead((*ZigParser).scanTypeMemberStart)
// 	case PCClassMembers:
// 		// We allow semicolons as class elements (as specified by ES6) as long as we're
// 		// not in error recovery.  If we're in error recovery, we don't want an errant
// 		// semicolon to be treated as a class member (since they're almost always used
// 		// for statements.
// 		return p.lookAhead((*ZigParser).scanClassMemberStart) || p.token == ast.KindSemicolonToken && !inErrorRecovery
// 	case PCEnumMembers:
...
// 		if p.token == ast.KindFromKeyword && p.lookAhead((*ZigParser).nextTokenIsTokenStringLiteral) {
// 			return false
// 		}
...
// func (p *ZigParser) isListTerminator(kind ParsingContext) bool {
// 	if p.token == ast.KindEndOfFile {
// 		return true
// 	}
// 	switch kind {
...
// 	case PCJsxChildren:
// 		return p.token == ast.KindLessThanToken && p.lookAhead((*ZigParser).nextTokenIsSlash)
// 	}
// 	return false
// }

// func (p *ZigParser) parseExpectedJSDoc(kind ast.Kind) bool {
// 	if p.token == kind {
// 		p.nextTokenJSDoc()
// 		return true
// 	}
...
// func (p *ZigParser) parseExpectedMatchingBrackets(openKind ast.Kind, closeKind ast.Kind, openParsed bool, openPosition int) {
...
// func (p *ZigParser) parseOptional(token ast.Kind) bool {
// 	if p.token == token {
// 		p.nextToken()
// 		return true
// 	}
...
// func (p *ZigParser) parseExpected(kind ast.Kind) bool {
// 	return p.parseExpectedWithDiagnostic(kind, nil, true)
// }

// func (p *ZigParser) parseExpectedWithoutAdvancing(kind ast.Kind) bool {
// 	return p.parseExpectedWithDiagnostic(kind, nil, false)
// }

// func (p *ZigParser) parseExpectedWithDiagnostic(kind ast.Kind, message *diagnostics.Message, shouldAdvance bool) bool {
// 	if p.token == kind {
// 		if shouldAdvance {
// 			p.nextToken()
// 		}
...
// func (p *ZigParser) parseTokenNode() *ast.Node {
// 	pos := p.nodePos()
// 	kind := p.token
// 	p.nextToken()
...
// func (p *ZigParser) parseExpectedToken(kind ast.Kind) *ast.Node {
...
// func (p *ZigParser) parseOptionalToken(kind ast.Kind) *ast.Node {
// 	if p.token == kind {
...
// func (p *ZigParser) parseExpectedTokenJSDoc(kind ast.Kind) *ast.Node {
...
// func (p *ZigParser) parseOptionalTokenJSDoc(kind ast.Kind) *ast.Node {
// 	if p.token == kind {
// 		return p.parseTokenNode()
// 	}
// 	return nil
...
// func (p *ZigParser) parseStatement() *ast.Statement {
// 	switch p.token {
...
// func (p *ZigParser) parseDeclaration() *ast.Statement {
...
// func (p *ZigParser) parseDeclarationWorker(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Statement {
// 	switch p.token {
...
// func (p *ZigParser) isLetDeclaration() bool {
// 	// In ES6 'let' always starts a lexical declaration if followed by an identifier or {
// 	// or [.
// 	return p.lookAhead((*ZigParser).nextTokenIsBindingIdentifierOrStartOfDestructuring)
// }

// func (p *ZigParser) nextTokenIsBindingIdentifierOrStartOfDestructuring() bool {
// 	p.nextToken()
// 	return p.isBindingIdentifier() || p.token == ast.KindOpenBraceToken || p.token == ast.KindOpenBracketToken
// }

// func (p *ZigParser) parseBlock(ignoreMissingOpenBrace bool, diagnosticMessage *diagnostics.Message) *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// 		statements := p.parseList(PCBlockStatements, (*ZigParser).parseStatement)
...
// func (p *ZigParser) parseEmptyStatement() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// func (p *ZigParser) parseIfStatement() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// func (p *ZigParser) parseDoStatement() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// func (p *ZigParser) parseWhileStatement() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
// 	p.parseExpected(ast.KindWhileKeyword)
// 	openParenPosition := p.scanner.TokenStart()
...
// func (p *ZigParser) parseForOrForInOrForOfStatement() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// 			p.token == ast.KindUsingKeyword && p.lookAhead((*ZigParser).nextTokenIsBindingIdentifierOrStartOfDestructuringOnSameLineDisallowOf) ||
// 			// this one is meant to allow of
// 			p.token == ast.KindAwaitKeyword && p.lookAhead((*ZigParser).nextIsUsingKeywordThenBindingIdentifierOrStartOfObjectDestructuringOnSameLine) {
// 			initializer = p.parseVariableDeclarationList(true /*inForStatementInitializer*/)
// 		} else {
// 			initializer = p.doInContext(ast.NodeFlagsDisallowInContext, true, (*ZigParser).parseExpression)
// 		}
// 	}
// 	var result *ast.Statement
// 	switch {
// 	case awaitToken != nil && p.parseExpected(ast.KindOfKeyword) || awaitToken == nil && p.parseOptional(ast.KindOfKeyword):
// 		expression := p.doInContext(ast.NodeFlagsDisallowInContext, false, (*ZigParser).parseAssignmentExpressionOrHigher)
...
// func (p *ZigParser) parseBreakStatement() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// func (p *ZigParser) parseContinueStatement() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// func (p *ZigParser) parseIdentifierUnlessAtSemicolon() *ast.Node {
...
// func (p *ZigParser) parseReturnStatement() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// func (p *ZigParser) parseWithStatement() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// 	expression := p.parseExpressionAllowIn()
// 	p.parseExpectedMatchingBrackets(ast.KindOpenParenToken, ast.KindCloseParenToken, openParenParsed, openParenPosition)
// 	statement := p.doInContext(ast.NodeFlagsInWithStatement, true, (*ZigParser).parseStatement)
// 	result := p.finishNode(p.factory.NewWithStatement(expression, statement), pos)
// 	p.withJSDoc(result, jsdoc)
// 	return result
// }

// func (p *ZigParser) parseCaseClause() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
// 	p.parseExpected(ast.KindCaseKeyword)
// 	expression := p.parseExpressionAllowIn()
// 	p.parseExpected(ast.KindColonToken)
// 	statements := p.parseList(PCSwitchClauseStatements, (*ZigParser).parseStatement)
// 	result := p.finishNode(p.factory.NewCaseOrDefaultClause(ast.KindCaseClause, expression, statements), pos)
// 	p.withJSDoc(result, jsdoc)
// 	return result
// }

// func (p *ZigParser) parseDefaultClause() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// func (p *ZigParser) parseCaseOrDefaultClause() *ast.Node {
...
// func (p *ZigParser) parseCaseBlock() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
// 	p.parseExpected(ast.KindOpenBraceToken)
// 	clauses := p.parseList(PCSwitchClauses, (*ZigParser).parseCaseOrDefaultClause)
...
// func (p *ZigParser) parseSwitchStatement() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// func (p *ZigParser) parseThrowStatement() *ast.Node {
...
// func (p *ZigParser) parseTryStatement() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// func (p *ZigParser) parseCatchClause() *ast.Node {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseDebuggerStatement() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// func (p *ZigParser) parseExpressionOrLabeledStatement() *ast.Statement {
...
// func (p *ZigParser) parseVariableStatement(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Node {
...
// func (p *ZigParser) parseVariableDeclarationList(inForStatementInitializer bool) *ast.Node {
// 	pos := p.nodePos()
...
// 	if p.token == ast.KindOfKeyword && p.lookAhead((*ZigParser).nextIsIdentifierAndCloseParen) {
// 		declarations = p.createMissingList()
// 	} else {
// 		saveContextFlags := p.contextFlags
// 		p.setContextFlags(ast.NodeFlagsDisallowInContext, inForStatementInitializer)
// 		declarations = p.parseDelimitedList(PCVariableDeclarations, core.IfElse(inForStatementInitializer, (*ZigParser).parseVariableDeclaration, (*ZigParser).parseVariableDeclarationAllowExclamation))
// 		p.contextFlags = saveContextFlags
// 	}
...
// func (p *ZigParser) nextIsIdentifierAndCloseParen() bool {
// 	return p.nextTokenIsIdentifier() && p.nextToken() == ast.KindCloseParenToken
// }

// func (p *ZigParser) nextTokenIsIdentifier() bool {
// 	p.nextToken()
// 	return p.isIdentifier()
// }

// func (p *ZigParser) parseVariableDeclaration() *ast.Node {
// 	return p.parseVariableDeclarationWorker(false /*allowExclamation*/)
// }

// func (p *ZigParser) parseVariableDeclarationAllowExclamation() *ast.Node {
// 	return p.parseVariableDeclarationWorker(true /*allowExclamation*/)
// }

// func (p *ZigParser) parseVariableDeclarationWorker(allowExclamation bool) *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// func (p *ZigParser) parseIdentifierOrPattern() *ast.Node {
// 	return p.parseIdentifierOrPatternWithDiagnostic(nil)
// }

// func (p *ZigParser) parseIdentifierOrPatternWithDiagnostic(privateIdentifierDiagnosticMessage *diagnostics.Message) *ast.Node {
...
// func (p *ZigParser) parseArrayBindingPattern() *ast.Node {
// 	pos := p.nodePos()
// 	p.parseExpected(ast.KindOpenBracketToken)
// 	saveContextFlags := p.contextFlags
// 	p.setContextFlags(ast.NodeFlagsDisallowInContext, false)
// 	elements := p.parseDelimitedList(PCArrayBindingElements, (*ZigParser).parseArrayBindingElement)
// 	p.contextFlags = saveContextFlags
...
// func (p *ZigParser) parseArrayBindingElement() *ast.Node {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseObjectBindingPattern() *ast.Node {
// 	pos := p.nodePos()
// 	p.parseExpected(ast.KindOpenBraceToken)
// 	saveContextFlags := p.contextFlags
// 	p.setContextFlags(ast.NodeFlagsDisallowInContext, false)
// 	elements := p.parseDelimitedList(PCObjectBindingElements, (*ZigParser).parseObjectBindingElement)
// 	p.contextFlags = saveContextFlags
// 	p.parseExpected(ast.KindCloseBraceToken)
// 	return p.finishNode(p.factory.NewBindingPattern(ast.KindObjectBindingPattern, elements), pos)
// }

// func (p *ZigParser) parseObjectBindingElement() *ast.Node {
// 	pos := p.nodePos()
// 	dotDotDotToken := p.parseOptionalToken(ast.KindDotDotDotToken)
...
// func (p *ZigParser) parseInitializer() *ast.Expression {
...
// func (p *ZigParser) parseTypeAnnotation() *ast.TypeNode {
...
// func (p *ZigParser) parseFunctionDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Node {
...
// func (p *ZigParser) parseClassDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Node {
// 	return p.parseClassDeclarationOrExpression(pos, jsdoc, modifiers, ast.KindClassDeclaration)
// }

// func (p *ZigParser) parseClassExpression() *ast.Node {
// 	return p.parseClassDeclarationOrExpression(p.nodePos(), p.jsdocScannerInfo(), nil /*modifiers*/, ast.KindClassExpression)
// }

// func (p *ZigParser) parseClassDeclarationOrExpression(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList, kind ast.Kind) *ast.Node {
// 	saveContextFlags := p.contextFlags
...
// 		members = p.parseList(PCClassMembers, (*ZigParser).parseClassElement)
// 		p.parseExpected(ast.KindCloseBraceToken)
// 	} else {
...
// func (p *ZigParser) parseNameOfClassDeclarationOrExpression() *ast.Node {
...
// func (p *ZigParser) isImplementsClause() bool {
// 	return p.token == ast.KindImplementsKeyword && p.lookAhead((*ZigParser).nextTokenIsIdentifierOrKeyword)
// }
...
// func (p *ZigParser) parseHeritageClauses(isInterface bool) *ast.NodeList {
// 	// ClassTail[Yield,Await] : (Modified) See 14.5
// 	//      ClassHeritage[?Yield,?Await]opt { ClassBody[?Yield,?Await]opt }
// 	if p.isHeritageClause() {
// 		return p.parseList(PCHeritageClauses, func(p *ZigParser) *ast.Node {
...
// func (p *ZigParser) parseHeritageClause(isInterface bool) *ast.Node {
// 	pos := p.nodePos()
// 	kind := p.token
// 	p.nextToken()
// 	parseElement := (*ZigParser).parseExpressionWithTypeArguments
// 	if isTypeHeritageClause(isInterface, kind) {
// 		parseElement = (*ZigParser).parseTypeHeritageClauseElement
// 	}
...
// func (p *ZigParser) parseTypeHeritageClauseElement() *ast.HeritageClauseElement {
// 	pos := p.nodePos()
...
// func (p *ZigParser) convertEntityNameExpressionToEntityName(node *ast.Node) *ast.Node {
// 	if ast.IsIdentifier(node) {
// 		return node
// 	}
...
// func (p *ZigParser) parseExpressionWithTypeArguments() *ast.Node {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseClassElement() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
// 	if p.token == ast.KindSemicolonToken {
// 		p.nextToken()
...
// 	if p.token == ast.KindStaticKeyword && p.lookAhead((*ZigParser).nextTokenIsOpenBrace) {
...
// func (p *ZigParser) parseClassStaticBlockDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Node {
...
// func (p *ZigParser) parseClassStaticBlockBody() *ast.Node {
// 	saveContextFlags := p.contextFlags
...
// func (p *ZigParser) tryParseConstructorDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Node {
// 	state := p.mark()
// 	if p.token == ast.KindConstructorKeyword || p.token == ast.KindStringLiteral && p.scanner.TokenValue() == "constructor" && p.lookAhead((*ZigParser).nextTokenIsOpenParen) {
// 		p.nextToken()
// 		typeParameters := p.parseTypeParameters()
...
// func (p *ZigParser) nextTokenIsOpenParen() bool {
// 	return p.nextToken() == ast.KindOpenParenToken
// }

// func (p *ZigParser) parsePropertyOrMethodDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Node {
// 	asteriskToken := p.parseOptionalToken(ast.KindAsteriskToken)
...
// func (p *ZigParser) parseMethodDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList, asteriskToken *ast.Node, name *ast.Node, questionToken *ast.Node, diagnosticMessage *diagnostics.Message) *ast.Node {
...
// func (p *ZigParser) parsePropertyDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList, name *ast.Node, questionToken *ast.Node) *ast.Node {
...
// 	}
// 	typeNode := p.parseTypeAnnotation()
// 	initializer := p.doInContext(ast.NodeFlagsYieldContext|ast.NodeFlagsAwaitContext|ast.NodeFlagsDisallowInContext, false, (*ZigParser).parseInitializer)
...
// func (p *ZigParser) parseSemicolonAfterPropertyName(name *ast.Node, typeNode *ast.TypeNode, initializer *ast.Expression) {
...
// func (p *ZigParser) parseErrorForMissingSemicolonAfter(node *ast.Node) {
...
// func (p *ZigParser) parseErrorForInvalidName(nameDiagnostic *diagnostics.Message, blankDiagnostic *diagnostics.Message, tokenIfBlankName ast.Kind) {
...
// func (p *ZigParser) parseInterfaceDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Node {
...
// func (p *ZigParser) parseTypeAliasDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Node {
...
// 	if p.token == ast.KindIntrinsicKeyword && p.lookAhead((*ZigParser).nextIsNotDot) {
...
// func (p *ZigParser) nextIsNotDot() bool {
...
// func (p *ZigParser) parseEnumMember() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
// 	name := p.parsePropertyName()
// 	initializer := p.doInContext(ast.NodeFlagsDisallowInContext, false, (*ZigParser).parseInitializer)
// 	result := p.finishNode(p.factory.NewEnumMember(name, initializer), pos)
// 	p.withJSDoc(result, jsdoc)
// 	return result
// }

// func (p *ZigParser) parseEnumDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Node {
// 	saveHasAwaitIdentifier := p.statementHasAwaitIdentifier
...
// 		members = p.parseDelimitedList(PCEnumMembers, (*ZigParser).parseEnumMember)
// 		p.contextFlags = saveContextFlags
// 		p.parseExpected(ast.KindCloseBraceToken)
// 	} else {
// 		members = p.createMissingList()
...
// func (p *ZigParser) parseModuleDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Statement {
...
// func (p *ZigParser) isTopLevelModuleFragment(modifiers *ast.ModifierList) bool {
...
// 		p.lookAhead((*ZigParser).nextTokenIsOpenBrace)
// }

// func (p *ZigParser) parseAmbientExternalModuleDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Node {
// 	var name *ast.Node
// 	keyword := ast.KindModuleKeyword
// 	saveHasAwaitIdentifier := p.statementHasAwaitIdentifier
// 	if p.token == ast.KindGlobalKeyword {
...
// func (p *ZigParser) parseModuleBlock() *ast.Node {
// 	pos := p.nodePos()
// 	var statements *ast.NodeList
// 	if p.parseExpected(ast.KindOpenBraceToken) {
// 		statements = p.parseList(PCBlockStatements, (*ZigParser).parseStatement)
...
// func (p *ZigParser) parseModuleOrNamespaceDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList, nested bool, keyword ast.Kind) *ast.Node {
// 	saveHasAwaitIdentifier := p.statementHasAwaitIdentifier
// 	var name *ast.Node
// 	if nested {
...
// func (p *ZigParser) parseImportDeclarationOrImportEqualsDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Statement {
...
// 		(p.token != ast.KindFromKeyword || p.isIdentifier() && p.lookAhead((*ZigParser).nextTokenIsFromKeywordOrEqualsToken)) &&
...
// 		if p.token == ast.KindFromKeyword {
// 			shouldParseAsDeferModifier = !p.lookAhead((*ZigParser).nextTokenIsTokenStringLiteral)
// 		} else {
...
// func (p *ZigParser) nextTokenIsFromKeywordOrEqualsToken() bool {
// 	p.nextToken()
// 	return p.token == ast.KindFromKeyword || p.token == ast.KindEqualsToken
// }

// func (p *ZigParser) tokenAfterImportDefinitelyProducesImportDeclaration() bool {
// 	return p.token == ast.KindAsteriskToken || p.token == ast.KindOpenBraceToken
// }

// func (p *ZigParser) tokenAfterImportedIdentifierDefinitelyProducesImportDeclaration() bool {
// 	// In `import id ___`, the current token decides whether to produce
// 	// an ImportDeclaration or ImportEqualsDeclaration.
// 	return p.token == ast.KindCommaToken || p.token == ast.KindFromKeyword
// }

// func (p *ZigParser) parseImportEqualsDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList, identifier *ast.Node, isTypeOnly bool) *ast.Node {
// 	p.parseExpected(ast.KindEqualsToken)
...
// func (p *ZigParser) parseModuleReference() *ast.Node {
// 	if p.token == ast.KindRequireKeyword && p.lookAhead((*ZigParser).nextTokenIsOpenParen) {
...
// func (p *ZigParser) parseExternalModuleReference() *ast.Node {
// 	saveHasAwaitIdentifier := p.statementHasAwaitIdentifier
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseModuleSpecifier() *ast.Expression {
// 	if p.token == ast.KindStringLiteral {
...
// func (p *ZigParser) tryParseImportClause(identifier *ast.Node, pos int, phaseModifier ast.Kind, skipJSDocLeadingAsterisks bool) *ast.Node {
// 	// ImportDeclaration:
...
// func (p *ZigParser) parseImportClause(identifier *ast.Node, pos int, phaseModifier ast.Kind, skipJSDocLeadingAsterisks bool) *ast.Node {
// 	// ImportClause:
...
// func (p *ZigParser) parseNamespaceImport() *ast.Node {
// 	// NameSpaceImport:
// 	//  * as ImportedBinding
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseNamedImports() *ast.Node {
// 	pos := p.nodePos()
// 	// NamedImports:
// 	//  { }
// 	//  { ImportsList }
// 	//  { ImportsList, }
// 	imports := p.parseBracketedList(PCImportOrExportSpecifiers, (*ZigParser).parseImportSpecifier, ast.KindOpenBraceToken, ast.KindCloseBraceToken)
// 	return p.finishNode(p.factory.NewNamedImports(imports), pos)
// }

// func (p *ZigParser) parseImportSpecifier() *ast.Node {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseImportOrExportSpecifier(kind ast.Kind) (isTypeOnly bool, propertyName *ast.Node, name *ast.Node) {
// 	// ImportSpecifier:
// 	//   BindingIdentifier
...
// func (p *ZigParser) canParseModuleExportName() bool {
// 	return tokenIsIdentifierOrKeyword(p.token) || p.token == ast.KindStringLiteral
// }

// func (p *ZigParser) parseModuleExportName(disallowKeywords bool) (node *ast.Node, nameOk bool) {
// 	nameOk = true
...
// func (p *ZigParser) tryParseImportAttributes() *ast.Node {
...
// func (p *ZigParser) parseExportAssignment(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Node {
// 	saveContextFlags := p.contextFlags
// 	saveHasAwaitIdentifier := p.statementHasAwaitIdentifier
// 	p.setContextFlags(ast.NodeFlagsAwaitContext, true)
...
// func (p *ZigParser) parseNamespaceExportDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Node {
// 	p.parseExpected(ast.KindAsKeyword)
...
// func (p *ZigParser) parseExportDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Node {
// 	saveContextFlags := p.contextFlags
// 	saveHasAwaitIdentifier := p.statementHasAwaitIdentifier
// 	p.setContextFlags(ast.NodeFlagsAwaitContext, true)
...
// func (p *ZigParser) parseNamespaceExport(pos int) *ast.Node {
...
// func (p *ZigParser) parseNamedExports() *ast.Node {
// 	pos := p.nodePos()
// 	// NamedImports:
// 	//  { }
// 	//  { ImportsList }
// 	//  { ImportsList, }
// 	exports := p.parseBracketedList(PCImportOrExportSpecifiers, (*ZigParser).parseExportSpecifier, ast.KindOpenBraceToken, ast.KindCloseBraceToken)
// 	return p.finishNode(p.factory.NewNamedExports(exports), pos)
// }

// func (p *ZigParser) parseExportSpecifier() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// func (p *ZigParser) parseType() *ast.TypeNode {
// 	saveContextFlags := p.contextFlags
...
// 			extendsType := p.doInContext(ast.NodeFlagsDisallowConditionalTypesContext, true, (*ZigParser).parseType)
// 			p.parseExpected(ast.KindQuestionToken)
// 			trueType := p.doInContext(ast.NodeFlagsDisallowConditionalTypesContext, false, (*ZigParser).parseType)
// 			p.parseExpected(ast.KindColonToken)
// 			falseType := p.doInContext(ast.NodeFlagsDisallowConditionalTypesContext, false, (*ZigParser).parseType)
...
// func (p *ZigParser) parseUnionTypeOrHigher() *ast.TypeNode {
// 	return p.parseUnionOrIntersectionType(ast.KindBarToken, (*ZigParser).parseIntersectionTypeOrHigher)
// }

// func (p *ZigParser) parseIntersectionTypeOrHigher() *ast.TypeNode {
// 	return p.parseUnionOrIntersectionType(ast.KindAmpersandToken, (*ZigParser).parseTypeOperatorOrHigher)
// }

// func (p *ZigParser) parseUnionOrIntersectionType(operator ast.Kind, parseConstituentType func(p *ZigParser) *ast.TypeNode) *ast.TypeNode {
// 	pos := p.nodePos()
...
// func (p *ZigParser) createUnionOrIntersectionTypeNode(operator ast.Kind, types *ast.NodeList) *ast.Node {
// 	switch operator {
...
// func (p *ZigParser) parseTypeOperatorOrHigher() *ast.TypeNode {
// 	operator := p.token
// 	switch operator {
...
// 	}
// 	return p.doInContext(ast.NodeFlagsDisallowConditionalTypesContext, false, (*ZigParser).parsePostfixTypeOrHigher)
// }

// func (p *ZigParser) parseTypeOperator(operator ast.Kind) *ast.Node {
// 	pos := p.nodePos()
// 	p.parseExpected(operator)
// 	return p.finishNode(p.factory.NewTypeOperatorNode(operator, p.parseTypeOperatorOrHigher()), pos)
// }

// func (p *ZigParser) parseInferType() *ast.Node {
// 	pos := p.nodePos()
// 	p.parseExpected(ast.KindInferKeyword)
// 	return p.finishNode(p.factory.NewInferTypeNode(p.parseTypeParameterOfInferType()), pos)
// }

// func (p *ZigParser) parseTypeParameterOfInferType() *ast.Node {
// 	pos := p.nodePos()
// 	name := p.parseIdentifier()
...
// func (p *ZigParser) tryParseConstraintOfInferType() *ast.Node {
// 	state := p.mark()
// 	if p.parseOptional(ast.KindExtendsKeyword) {
// 		constraint := p.doInContext(ast.NodeFlagsDisallowConditionalTypesContext, true, (*ZigParser).parseType)
...
// func (p *ZigParser) parsePostfixTypeOrHigher() *ast.Node {
// 	pos := p.nodePos()
...
// 			if p.lookAhead((*ZigParser).nextIsStartOfType) {
// 				return typeNode
// 			}
// 			p.nextToken()
...
// func (p *ZigParser) nextIsStartOfType() bool {
// 	p.nextToken()
// 	return p.isStartOfType(false /*inStartOfParameter*/)
// }

// func (p *ZigParser) parseNonArrayType() *ast.Node {
// 	switch p.token {
...
// 	case ast.KindMinusToken:
// 		if p.lookAhead((*ZigParser).nextTokenIsNumericOrBigIntLiteral) {
...
// 		if p.lookAhead((*ZigParser).nextIsStartOfTypeOfImportType) {
// 			return p.parseImportType()
// 		}
// 		return p.parseTypeQuery()
// 	case ast.KindOpenBraceToken:
// 		if p.lookAhead((*ZigParser).nextIsStartOfMappedType) {
...
// 		if p.lookAhead((*ZigParser).nextTokenIsIdentifierOrKeywordOnSameLine) {
...
// func (p *ZigParser) parseKeywordTypeNode() *ast.Node {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseJSDocType() *ast.TypeNode {
// 	p.scanner.SetSkipJSDocLeadingAsterisks(true)
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseLiteralTypeNode(negative bool) *ast.Node {
// 	pos := p.nodePos()
// 	if negative {
// 		p.nextToken()
// 	}
...
// func (p *ZigParser) parseTypeReference() *ast.Node {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseEntityNameOfTypeReference() *ast.Node {
// 	return p.parseEntityName(true /*allowReservedWords*/, false /*allowPrivateName*/, diagnostics.Type_expected)
// }

// func (p *ZigParser) parseEntityName(allowReservedWords bool, allowPrivateName bool, diagnosticMessage *diagnostics.Message) *ast.Node {
// 	pos := p.nodePos()
// 	var entity *ast.Node
...
// func (p *ZigParser) parseRightSideOfDot(allowIdentifierNames bool, allowPrivateIdentifiers bool, allowUnicodeEscapeSequenceInIdentifierName bool) *ast.Node {
...
// 	if p.hasPrecedingLineBreak() && tokenIsIdentifierOrKeyword(p.token) && p.lookAhead((*ZigParser).nextTokenIsIdentifierOrKeywordOnSameLine) {
...
// func (p *ZigParser) newIdentifier(text string) *ast.Node {
// 	p.identifierCount++
...
// func (p *ZigParser) createMissingIdentifier() *ast.Node {
// 	return p.finishNode(p.newIdentifier(""), p.nodePos())
// }

// func (p *ZigParser) parsePrivateIdentifier() *ast.Node {
// 	pos := p.nodePos()
...
// func (p *ZigParser) reScanLessThanToken() ast.Kind {
// 	p.token = p.scanner.ReScanLessThanToken()
// 	return p.token
// }

// func (p *ZigParser) reScanGreaterThanToken() ast.Kind {
// 	p.token = p.scanner.ReScanGreaterThanToken()
// 	return p.token
// }

// func (p *ZigParser) reScanSlashToken() ast.Kind {
// 	p.token = p.scanner.ReScanSlashToken()
// 	return p.token
// }

// func (p *ZigParser) reScanTemplateToken(isTaggedTemplate bool) ast.Kind {
// 	p.token = p.scanner.ReScanTemplateToken(isTaggedTemplate)
// 	return p.token
// }

// func (p *ZigParser) parseTypeArgumentsOfTypeReference() *ast.NodeList {
...
// func (p *ZigParser) parseTypeArguments() *ast.NodeList {
// 	if p.token == ast.KindLessThanToken {
// 		return p.parseBracketedList(PCTypeArguments, (*ZigParser).parseType, ast.KindLessThanToken, ast.KindGreaterThanToken)
// 	}
// 	return nil
// }

// func (p *ZigParser) nextIsStartOfTypeOfImportType() bool {
// 	p.nextToken()
// 	return p.token == ast.KindImportKeyword
// }

// func (p *ZigParser) parseImportType() *ast.Node {
...
// func (p *ZigParser) parseImportAttribute() *ast.Node {
// 	pos := p.nodePos()
// 	var name *ast.Node
...
// func (p *ZigParser) parseImportAttributes(token ast.Kind, skipKeyword bool) *ast.Node {
// 	pos := p.nodePos()
// 	if !skipKeyword {
...
// 	if p.parseExpected(ast.KindOpenBraceToken) {
// 		multiLine = p.hasPrecedingLineBreak()
// 		elements = p.parseDelimitedList(PCImportAttributes, (*ZigParser).parseImportAttribute)
// 		if !p.parseExpected(ast.KindCloseBraceToken) {
// 			if len(p.diagnostics) != 0 {
// 				lastDiagnostic := p.diagnostics[len(p.diagnostics)-1]
// 				if lastDiagnostic.Code() == diagnostics.X_0_expected.Code() {
...
// func (p *ZigParser) parseTypeQuery() *ast.Node {
// 	pos := p.nodePos()
...
// func (p *ZigParser) nextIsStartOfMappedType() bool {
// 	p.nextToken()
...
// func (p *ZigParser) parseMappedType() *ast.Node {
// 	pos := p.nodePos()
// 	p.parseExpected(ast.KindOpenBraceToken)
...
// func (p *ZigParser) parseTypeMember() *ast.Node {
// 	if p.token == ast.KindOpenParenToken || p.token == ast.KindLessThanToken {
// 		return p.parseSignatureMember(ast.KindCallSignature)
// 	}
// 	if p.token == ast.KindNewKeyword && p.lookAhead((*ZigParser).nextTokenIsOpenParenOrLessThan) {
...
// func (p *ZigParser) nextTokenIsOpenParenOrLessThan() bool {
// 	p.nextToken()
// 	return p.token == ast.KindOpenParenToken || p.token == ast.KindLessThanToken
// }

// func (p *ZigParser) parseSignatureMember(kind ast.Kind) *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// func (p *ZigParser) parseTypeParameters() *ast.NodeList {
// 	if p.token == ast.KindLessThanToken {
// 		return p.parseBracketedList(PCTypeParameters, (*ZigParser).parseTypeParameter, ast.KindLessThanToken, ast.KindGreaterThanToken)
// 	}
// 	return nil
// }

// func (p *ZigParser) parseTypeParameter() *ast.Node {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseParameters(flags ParseFlags) *ast.NodeList {
...
// func (p *ZigParser) parseParametersWorker(flags ParseFlags, allowAmbiguity bool) *ast.NodeList {
// 	// FormalParameters [Yield,Await]: (modified)
// 	//      [empty]
// 	//      FormalParameterList[?Yield,Await]
// 	//
...
// 	parameters := p.parseDelimitedList(PCParameters, func(p *ZigParser) *ast.Node {
...
// func (p *ZigParser) parseParameter() *ast.Node {
// 	return p.parseParameterEx(false /*inOuterAwaitContext*/, true /*allowAmbiguity*/)
// }

// func (p *ZigParser) parseParameterEx(inOuterAwaitContext bool, allowAmbiguity bool) *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// func (p *ZigParser) isParameterNameStart() bool {
...
// func (p *ZigParser) parseNameOfParameter(modifiers *ast.ModifierList) *ast.Node {
// 	// FormalParameter [Yield,Await]:
// 	//      BindingElement[?Yield,?Await]
...
// func (p *ZigParser) parseReturnType(returnToken ast.Kind, isType bool) *ast.TypeNode {
// 	if p.shouldParseReturnType(returnToken, isType) {
// 		return p.doInContext(ast.NodeFlagsDisallowConditionalTypesContext, false, (*ZigParser).parseTypeOrTypePredicate)
// 	}
// 	return nil
// }

// func (p *ZigParser) shouldParseReturnType(returnToken ast.Kind, isType bool) bool {
...
// func (p *ZigParser) parseTypeOrTypePredicate() *ast.TypeNode {
// 	if p.isIdentifier() {
// 		state := p.mark()
// 		pos := p.nodePos()
// 		id := p.parseIdentifier()
...
// func (p *ZigParser) parseTypeMemberSemicolon() {
...
// func (p *ZigParser) parseAccessorDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList, kind ast.Kind, flags ParseFlags) *ast.Node {
// 	name := p.parsePropertyName()
// 	typeParameters := p.parseTypeParameters()
// 	parameters := p.parseParameters(ParseFlagsNone)
// 	returnType := p.parseReturnType(ast.KindColonToken, false /*isType*/)
...
// func (p *ZigParser) parsePropertyName() *ast.Node {
// 	saveHasAwaitIdentifier := p.statementHasAwaitIdentifier
...
// func (p *ZigParser) parsePropertyNameWorker(allowComputedPropertyNames bool) *ast.Node {
...
// func (p *ZigParser) parseComputedPropertyName() *ast.Node {
// 	// PropertyName [Yield]:
// 	//      LiteralPropertyName
...
// func (p *ZigParser) parseFunctionBlockOrSemicolon(flags ParseFlags, diagnosticMessage *diagnostics.Message) *ast.Node {
// 	if p.token != ast.KindOpenBraceToken {
// 		if flags&ParseFlagsType != 0 {
// 			p.parseTypeMemberSemicolon()
// 			return nil
...
// func (p *ZigParser) parseFunctionBlock(flags ParseFlags, diagnosticMessage *diagnostics.Message) *ast.Node {
// 	saveContextFlags := p.contextFlags
// 	saveHasAwaitIdentifier := p.statementHasAwaitIdentifier
// 	p.setContextFlags(ast.NodeFlagsYieldContext, flags&ParseFlagsYield != 0)
// 	p.setContextFlags(ast.NodeFlagsAwaitContext, flags&ParseFlagsAwait != 0)
...
// func (p *ZigParser) isIndexSignature() bool {
// 	return p.token == ast.KindOpenBracketToken && p.lookAhead((*ZigParser).nextIsUnambiguouslyIndexSignature)
// }

// func (p *ZigParser) nextIsUnambiguouslyIndexSignature() bool {
...
// func (p *ZigParser) parseIndexSignatureDeclaration(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Node {
// 	parameters := p.parseBracketedList(PCParameters, (*ZigParser).parseParameter, ast.KindOpenBracketToken, ast.KindCloseBracketToken)
// 	typeNode := p.parseTypeAnnotation()
// 	p.parseTypeMemberSemicolon()
...
// func (p *ZigParser) parsePropertyOrMethodSignature(pos int, jsdoc jsdocScannerInfo, modifiers *ast.ModifierList) *ast.Node {
// 	name := p.parsePropertyName()
// 	questionToken := p.parseOptionalToken(ast.KindQuestionToken)
// 	var result *ast.Node
// 	if p.token == ast.KindOpenParenToken || p.token == ast.KindLessThanToken {
...
// func (p *ZigParser) parseTypeLiteral() *ast.Node {
// 	pos := p.nodePos()
// 	result := p.finishNode(p.factory.NewTypeLiteralNode(p.parseObjectTypeMembers()), pos)
// 	return result
// }

// func (p *ZigParser) parseObjectTypeMembers() *ast.NodeList {
// 	if p.parseExpected(ast.KindOpenBraceToken) {
// 		members := p.parseList(PCTypeMembers, (*ZigParser).parseTypeMember)
// 		p.parseExpected(ast.KindCloseBraceToken)
// 		return members
...
// func (p *ZigParser) parseTupleType() *ast.Node {
// 	pos := p.nodePos()
// 	return p.finishNode(p.factory.NewTupleTypeNode(p.parseBracketedList(PCTupleElementTypes, (*ZigParser).parseTupleElementNameOrTupleElementType, ast.KindOpenBracketToken, ast.KindCloseBracketToken)), pos)
// }

// func (p *ZigParser) parseTupleElementNameOrTupleElementType() *ast.Node {
// 	if p.lookAhead((*ZigParser).scanStartOfNamedTupleElement) {
// 		pos := p.nodePos()
// 		jsdoc := p.jsdocScannerInfo()
// 		dotDotDotToken := p.parseOptionalToken(ast.KindDotDotDotToken)
// 		name := p.parseIdentifierName()
...
// func (p *ZigParser) scanStartOfNamedTupleElement() bool {
...
// func (p *ZigParser) nextTokenIsColonOrQuestionColon() bool {
// 	return p.nextToken() == ast.KindColonToken || p.token == ast.KindQuestionToken && p.nextToken() == ast.KindColonToken
// }

// func (p *ZigParser) parseTupleElementType() *ast.TypeNode {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseParenthesizedType() *ast.Node {
// 	pos := p.nodePos()
// 	p.parseExpected(ast.KindOpenParenToken)
// 	typeNode := p.parseType()
// 	p.parseExpected(ast.KindCloseParenToken)
...
// func (p *ZigParser) parseAssertsTypePredicate() *ast.TypeNode {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseTemplateType() *ast.Node {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseTemplateHead(isTaggedTemplate bool) *ast.Node {
...
// func (p *ZigParser) getTemplateLiteralRawText(endLength int) string {
...
// func (p *ZigParser) parseTemplateTypeSpans() *ast.NodeList {
// 	pos := p.nodePos()
// 	var list []*ast.Node
// 	for {
...
// func (p *ZigParser) parseTemplateTypeSpan() *ast.Node {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseLiteralOfTemplateSpan(isTaggedTemplate bool) *ast.Node {
...
// func (p *ZigParser) parseTemplateMiddleOrTail() *ast.Node {
// 	pos := p.nodePos()
// 	var result *ast.Node
...
// func (p *ZigParser) parseFunctionOrConstructorTypeToError(isInUnionType bool, parseConstituentType func(p *ZigParser) *ast.TypeNode) *ast.TypeNode {
...
// func (p *ZigParser) isStartOfFunctionTypeOrConstructorType() bool {
// 	return p.token == ast.KindLessThanToken ||
// 		p.token == ast.KindOpenParenToken && p.lookAhead((*ZigParser).nextIsUnambiguouslyStartOfFunctionType) ||
// 		p.token == ast.KindNewKeyword ||
// 		p.token == ast.KindAbstractKeyword && p.lookAhead((*ZigParser).nextTokenIsNewKeyword)
// }

// func (p *ZigParser) parseFunctionOrConstructorType() *ast.TypeNode {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// func (p *ZigParser) parseModifiersForConstructorType() *ast.ModifierList {
...
// func (p *ZigParser) nextTokenIsNewKeyword() bool {
// 	return p.nextToken() == ast.KindNewKeyword
// }

// func (p *ZigParser) nextIsUnambiguouslyStartOfFunctionType() bool {
// 	p.nextToken()
...
// func (p *ZigParser) skipParameterStart() bool {
// 	if ast.IsModifierKind(p.token) {
// 		// Skip modifiers
// 		p.parseModifiers()
// 	}
...
// func (p *ZigParser) parseModifiers() *ast.ModifierList {
// 	return p.parseModifiersEx(false, false, false)
// }

// func (p *ZigParser) parseModifiersEx(allowDecorators bool, permitConstAsModifier bool, stopOnStartOfClassStaticBlock bool) *ast.ModifierList {
...
// func (p *ZigParser) parseDecorator() *ast.Node {
// 	pos := p.nodePos()
// 	p.parseExpected(ast.KindAtToken)
// 	expression := p.doInContext(ast.NodeFlagsDecoratorContext, true, (*ZigParser).parseDecoratorExpression)
// 	return p.finishNode(p.factory.NewDecorator(expression), pos)
// }

// func (p *ZigParser) parseDecoratorExpression() *ast.Expression {
...
// func (p *ZigParser) tryParseModifier(hasSeenStaticModifier bool, permitConstAsModifier bool, stopOnStartOfClassStaticBlock bool) *ast.Node {
// 	pos := p.nodePos()
// 	kind := p.token
// 	if p.token == ast.KindConstKeyword && permitConstAsModifier {
// 		// We need to ensure that any subsequent modifiers appear on the same line
// 		// so that when 'const' is a standalone declaration, we don't issue an error.
// 		if !p.lookAhead((*ZigParser).nextTokenIsOnSameLineAndCanFollowModifier) {
// 			return nil
// 		} else {
// 			p.nextToken()
// 		}
// 	} else if stopOnStartOfClassStaticBlock && p.token == ast.KindStaticKeyword && p.lookAhead((*ZigParser).nextTokenIsOpenBrace) {
// 		return nil
...
// func (p *ZigParser) parseContextualModifier(t ast.Kind) bool {
// 	state := p.mark()
...
// func (p *ZigParser) parseAnyContextualModifier() bool {
// 	state := p.mark()
...
// func (p *ZigParser) nextTokenCanFollowModifier() bool {
// 	switch p.token {
// 	case ast.KindConstKeyword:
...
// 		p.nextToken()
// 		if p.token == ast.KindDefaultKeyword {
// 			return p.lookAhead((*ZigParser).nextTokenCanFollowDefaultKeyword)
// 		}
// 		if p.token == ast.KindTypeKeyword {
// 			return p.lookAhead((*ZigParser).nextTokenCanFollowExportModifier)
// 		}
...
// func (p *ZigParser) nextTokenCanFollowDefaultKeyword() bool {
// 	switch p.nextToken() {
// 	case ast.KindClassKeyword, ast.KindFunctionKeyword, ast.KindInterfaceKeyword, ast.KindAtToken:
// 		return true
// 	case ast.KindAbstractKeyword:
// 		return p.lookAhead((*ZigParser).nextTokenIsClassKeywordOnSameLine)
// 	case ast.KindAsyncKeyword:
// 		return p.lookAhead((*ZigParser).nextTokenIsFunctionKeywordOnSameLine)
// 	}
// 	return false
// }

// func (p *ZigParser) nextTokenIsIdentifierOrKeyword() bool {
// 	return tokenIsIdentifierOrKeyword(p.nextToken())
// }

// func (p *ZigParser) nextTokenIsIdentifierOrKeywordOrGreaterThan() bool {
// 	return tokenIsIdentifierOrKeywordOrGreaterThan(p.nextToken())
// }

// func (p *ZigParser) nextTokenIsIdentifierOrKeywordOnSameLine() bool {
// 	return p.nextTokenIsIdentifierOrKeyword() && !p.hasPrecedingLineBreak()
// }

// func (p *ZigParser) nextTokenIsIdentifierOrKeywordOrLiteralOnSameLine() bool {
...
// func (p *ZigParser) nextTokenIsClassKeywordOnSameLine() bool {
// 	return p.nextToken() == ast.KindClassKeyword && !p.hasPrecedingLineBreak()
// }

// func (p *ZigParser) nextTokenIsFunctionKeywordOnSameLine() bool {
// 	return p.nextToken() == ast.KindFunctionKeyword && !p.hasPrecedingLineBreak()
// }

// func (p *ZigParser) nextTokenCanFollowExportModifier() bool {
// 	p.nextToken()
// 	return p.canFollowExportModifier()
// }

// func (p *ZigParser) canFollowExportModifier() bool {
...
// func (p *ZigParser) canFollowModifier() bool {
...
// func (p *ZigParser) canFollowGetOrSetKeyword() bool {
// 	return p.token == ast.KindOpenBracketToken || p.isLiteralPropertyName()
// }

// func (p *ZigParser) nextTokenIsOnSameLineAndCanFollowModifier() bool {
// 	p.nextToken()
// 	if p.hasPrecedingLineBreak() {
// 		return false
// 	}
...
// func (p *ZigParser) nextTokenIsOpenBrace() bool {
// 	return p.nextToken() == ast.KindOpenBraceToken
// }

// func (p *ZigParser) nextTokenIsOpenBraceOnSameLine() bool {
// 	return p.nextToken() == ast.KindOpenBraceToken && !p.hasPrecedingLineBreak()
// }

// func (p *ZigParser) parseExpression() *ast.Expression {
// 	// Expression[in]:
...
// func (p *ZigParser) parseExpressionAllowIn() *ast.Expression {
// 	return p.doInContext(ast.NodeFlagsDisallowInContext, false, (*ZigParser).parseExpression)
// }

// func (p *ZigParser) parseAssignmentExpressionOrHigher() *ast.Expression {
// 	return p.parseAssignmentExpressionOrHigherWorker(true /*allowReturnTypeInArrowFunction*/)
// }

// func (p *ZigParser) parseAssignmentExpressionOrHigherWorker(allowReturnTypeInArrowFunction bool) *ast.Expression {
...
// func (p *ZigParser) isYieldExpression() bool {
...
// 		return p.lookAhead((*ZigParser).nextTokenIsIdentifierOrKeywordOrLiteralOnSameLine)
// 	}
// 	return false
// }

// func (p *ZigParser) parseYieldExpression() *ast.Node {
// 	pos := p.nodePos()
// 	// YieldExpression[In] :
// 	//      yield
...
// func (p *ZigParser) isParenthesizedArrowFunctionExpression() core.Tristate {
...
// func (p *ZigParser) nextIsParenthesizedArrowFunctionExpression() core.Tristate {
...
// 		if ast.IsModifierKind(second) && second != ast.KindAsyncKeyword && p.lookAhead((*ZigParser).nextTokenIsIdentifier) {
...
// 			isArrowFunctionInJsx := p.lookAhead(func(p *ZigParser) bool {
...
// func (p *ZigParser) tryParseParenthesizedArrowFunctionExpression(allowReturnTypeInArrowFunction bool) *ast.Node {
...
// func (p *ZigParser) parseParenthesizedArrowFunctionExpression(allowAmbiguity bool, allowReturnTypeInArrowFunction bool) *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
...
// func (p *ZigParser) parseModifiersForArrowFunction() *ast.ModifierList {
// 	if p.token == ast.KindAsyncKeyword {
// 		pos := p.nodePos()
// 		p.nextToken()
...
// func (p *ZigParser) parseArrowFunctionExpressionBody(isAsync bool, allowReturnTypeInArrowFunction bool) *ast.Node {
// 	if p.token == ast.KindOpenBraceToken {
...
// func (p *ZigParser) isStartOfExpressionStatement() bool {
...
// func (p *ZigParser) parsePossibleParenthesizedArrowFunctionExpression(allowReturnTypeInArrowFunction bool) *ast.Node {
...
// func (p *ZigParser) tryParseAsyncSimpleArrowFunctionExpression(allowReturnTypeInArrowFunction bool) *ast.Node {
// 	// We do a check here so that we won't be doing unnecessarily call to "lookAhead"
// 	if p.token == ast.KindAsyncKeyword && p.lookAhead((*ZigParser).nextIsUnParenthesizedAsyncArrowFunction) {
// 		pos := p.nodePos()
// 		jsdoc := p.jsdocScannerInfo()
...
// func (p *ZigParser) nextIsUnParenthesizedAsyncArrowFunction() bool {
// 	// AsyncArrowFunctionExpression:
// 	//      1) async[no LineTerminator here]AsyncArrowBindingIdentifier[?Yield][no LineTerminator here]=>AsyncConciseBody[?In]
// 	//      2) CoverCallExpressionAndAsyncArrowHead[?Yield, ?Await][no LineTerminator here]=>AsyncConciseBody[?In]
// 	if p.token == ast.KindAsyncKeyword {
...
// func (p *ZigParser) parseSimpleArrowFunctionExpression(pos int, identifier *ast.Node, allowReturnTypeInArrowFunction bool, jsdoc jsdocScannerInfo, asyncModifier *ast.ModifierList) *ast.Node {
...
// func (p *ZigParser) parseConditionalExpressionRest(leftOperand *ast.Expression, pos int, allowReturnTypeInArrowFunction bool) *ast.Expression {
...
// func (p *ZigParser) parseBinaryExpressionOrHigher(precedence ast.OperatorPrecedence) *ast.Expression {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseBinaryExpressionRest(precedence ast.OperatorPrecedence, leftOperand *ast.Expression, pos int) *ast.Expression {
...
// func (p *ZigParser) makeSatisfiesExpression(expression *ast.Expression, typeNode *ast.TypeNode) *ast.Node {
// 	return p.checkJSSyntax(p.finishNode(p.factory.NewSatisfiesExpression(expression, typeNode), expression.Pos()))
// }

// func (p *ZigParser) makeAsExpression(left *ast.Expression, right *ast.TypeNode) *ast.Node {
// 	return p.checkJSSyntax(p.finishNode(p.factory.NewAsExpression(left, right), left.Pos()))
// }

// func (p *ZigParser) makeBinaryExpression(left *ast.Expression, operatorToken *ast.Node, right *ast.Expression, pos int) *ast.Node {
// 	return p.finishNode(p.factory.NewBinaryExpression(nil /*modifiers*/, left, nil /*typeNode*/, operatorToken, right), pos)
// }

// func (p *ZigParser) parseUnaryExpressionOrHigher() *ast.Expression {
...
// func (p *ZigParser) isUpdateExpression() bool {
// 	switch p.token {
...
// func (p *ZigParser) parseUpdateExpression() *ast.Expression {
// 	pos := p.nodePos()
...
// 	} else if p.languageVariant == core.LanguageVariantJSX && p.token == ast.KindLessThanToken && p.lookAhead((*ZigParser).nextTokenIsIdentifierOrKeywordOrGreaterThan) {
...
// func (p *ZigParser) parseJsxElementOrSelfClosingElementOrFragment(inExpressionContext bool, topInvalidNodePosition int, openingTag *ast.Node, mustBeUnary bool) *ast.Expression {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseJsxChildren(openingTag *ast.Expression) *ast.NodeList {
// 	pos := p.nodePos()
// 	saveParsingContexts := p.parsingContexts
...
// func (p *ZigParser) parseJsxChild(openingTag *ast.Node, token ast.Kind) *ast.Expression {
// 	switch token {
...
// func (p *ZigParser) parseJsxText() *ast.Node {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseJsxExpression(inExpressionContext bool) *ast.Node {
// 	pos := p.nodePos()
// 	if !p.parseExpected(ast.KindOpenBraceToken) {
// 		return nil
// 	}
...
// func (p *ZigParser) scanJsxText() ast.Kind {
// 	p.token = p.scanner.ScanJsxToken()
// 	return p.token
// }

// func (p *ZigParser) scanJsxIdentifier() ast.Kind {
// 	p.token = p.scanner.ScanJsxIdentifier()
// 	return p.token
// }

// func (p *ZigParser) scanJsxAttributeValue() ast.Kind {
// 	p.token = p.scanner.ScanJsxAttributeValue()
// 	return p.token
// }

// func (p *ZigParser) parseJsxClosingElement(open *ast.Node, inExpressionContext bool) *ast.Node {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseJsxOpeningOrSelfClosingElementOrOpeningFragment(inExpressionContext bool) *ast.Expression {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseJsxElementName() *ast.Expression {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseJsxTagName() *ast.Expression {
// 	pos := p.nodePos()
// 	p.scanJsxIdentifier()
...
// func (p *ZigParser) parseJsxAttributes() *ast.Node {
// 	pos := p.nodePos()
// 	return p.finishNode(p.factory.NewJsxAttributes(p.parseList(PCJsxAttributes, (*ZigParser).parseJsxAttribute)), pos)
// }

// func (p *ZigParser) parseJsxAttribute() *ast.Node {
// 	if p.token == ast.KindOpenBraceToken {
...
// func (p *ZigParser) parseJsxSpreadAttribute() *ast.Node {
// 	pos := p.nodePos()
// 	p.parseExpected(ast.KindOpenBraceToken)
...
// func (p *ZigParser) parseJsxAttributeName() *ast.Node {
// 	pos := p.nodePos()
// 	p.scanJsxIdentifier()
...
// func (p *ZigParser) parseJsxAttributeValue() *ast.Expression {
// 	if p.token == ast.KindEqualsToken {
...
// func (p *ZigParser) parseJsxClosingFragment(inExpressionContext bool) *ast.Node {
// 	pos := p.nodePos()
// 	p.parseExpected(ast.KindLessThanSlashToken)
...
// func (p *ZigParser) parseSimpleUnaryExpression() *ast.Expression {
// 	switch p.token {
...
// func (p *ZigParser) parsePrefixUnaryExpression() *ast.Node {
// 	pos := p.nodePos()
// 	operator := p.token
// 	p.nextToken()
...
// func (p *ZigParser) parseAwaitExpression() *ast.Node {
// 	pos := p.nodePos()
// 	p.nextToken()
// 	return p.finishNode(p.factory.NewAwaitExpression(p.parseSimpleUnaryExpression()), pos)
// }

// func (p *ZigParser) parseTypeAssertion() *ast.Node {
...
// func (p *ZigParser) parseLeftHandSideExpressionOrHigher() *ast.Expression {
// 	// Original Ecma:
...
// 		if p.lookAhead((*ZigParser).nextTokenIsOpenParenOrLessThan) {
...
// 			p.sourceFlags |= ast.NodeFlagsPossiblyContainsDynamicImport
// 			expression = p.parseKeywordExpression()
// 		} else if p.lookAhead((*ZigParser).nextTokenIsDot) {
...
// func (p *ZigParser) nextTokenIsDot() bool {
// 	return p.nextToken() == ast.KindDotToken
// }

// func (p *ZigParser) parseSuperExpression() *ast.Expression {
// 	pos := p.nodePos()
// 	expression := p.parseKeywordExpression()
// 	if p.token == ast.KindLessThanToken {
// 		startPos := p.nodePos()
...
// func (p *ZigParser) isTemplateStartOfTaggedTemplate() bool {
// 	return p.token == ast.KindNoSubstitutionTemplateLiteral || p.token == ast.KindTemplateHead
// }

// func (p *ZigParser) tryParseTypeArgumentsInExpression() *ast.NodeList {
...
// 		p.nextToken()
// 		typeArguments := p.parseDelimitedList(PCTypeArguments, (*ZigParser).parseType)
...
// func (p *ZigParser) canFollowTypeArgumentsInExpression() bool {
// 	switch p.token {
...
// func (p *ZigParser) parseMemberExpressionOrHigher() *ast.Node {
...
// func (p *ZigParser) parseMemberExpressionRest(pos int, expression *ast.Expression, allowOptionalChain bool) *ast.Expression {
// 	for {
...
// func (p *ZigParser) isStartOfOptionalPropertyOrElementAccessChain() bool {
// 	return p.token == ast.KindQuestionDotToken && p.lookAhead((*ZigParser).nextTokenIsIdentifierOrKeywordOrOpenBracketOrTemplate)
// }

// func (p *ZigParser) nextTokenIsIdentifierOrKeywordOrOpenBracketOrTemplate() bool {
// 	p.nextToken()
// 	return tokenIsIdentifierOrKeyword(p.token) || p.token == ast.KindOpenBracketToken || p.isTemplateStartOfTaggedTemplate()
// }

// func (p *ZigParser) parsePropertyAccessExpressionRest(pos int, expression *ast.Expression, questionDotToken *ast.Node) *ast.Node {
...
// func (p *ZigParser) tryReparseOptionalChain(node *ast.Expression) bool {
...
// func (p *ZigParser) parseElementAccessExpressionRest(pos int, expression *ast.Expression, questionDotToken *ast.Node) *ast.Node {
...
// func (p *ZigParser) parseCallExpressionRest(pos int, expression *ast.Expression) *ast.Expression {
// 	for {
...
// func (p *ZigParser) parseArgumentList() *ast.NodeList {
// 	p.parseExpected(ast.KindOpenParenToken)
// 	result := p.parseDelimitedList(PCArgumentExpressions, (*ZigParser).parseArgumentExpression)
// 	p.parseExpected(ast.KindCloseParenToken)
// 	return result
// }

// func (p *ZigParser) parseArgumentExpression() *ast.Expression {
// 	return p.doInContext(ast.NodeFlagsDisallowInContext|ast.NodeFlagsDecoratorContext, false, (*ZigParser).parseArgumentOrArrayLiteralElement)
// }

// func (p *ZigParser) parseArgumentOrArrayLiteralElement() *ast.Expression {
// 	switch p.token {
...
// func (p *ZigParser) parseSpreadElement() *ast.Node {
// 	pos := p.nodePos()
// 	p.parseExpected(ast.KindDotDotDotToken)
// 	expression := p.parseAssignmentExpressionOrHigher()
...
// func (p *ZigParser) parseTaggedTemplateRest(pos int, tag *ast.Expression, questionDotToken *ast.Node, typeArguments *ast.NodeList) *ast.Node {
...
// func (p *ZigParser) parseTemplateExpression(isTaggedTemplate bool) *ast.Expression {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseTemplateSpans(isTaggedTemplate bool) *ast.NodeList {
// 	pos := p.nodePos()
// 	var list []*ast.Node
// 	for {
...
// func (p *ZigParser) parseTemplateSpan(isTaggedTemplate bool) *ast.Node {
// 	pos := p.nodePos()
// 	expression := p.parseExpressionAllowIn()
...
// func (p *ZigParser) parsePrimaryExpression() *ast.Expression {
// 	switch p.token {
...
// 		if !p.lookAhead((*ZigParser).nextTokenIsFunctionKeywordOnSameLine) {
// 			break
// 		}
...
// 		if p.lookAhead((*ZigParser).nextTokenIsOpenBraceOnSameLine) {
...
// func (p *ZigParser) parseModuleExpression() *ast.Expression {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
// 	p.parseExpected(ast.KindModuleKeyword)
...
// 	savedInModuleFragment := p.inModuleFragment
// 	p.inModuleFragment = true
// 	body := p.doInContext(ast.NodeFlagsAwaitContext, true, func(p *ZigParser) *ast.Node {
...
// func (p *ZigParser) parseParenthesizedExpression() *ast.Expression {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
// 	p.parseExpected(ast.KindOpenParenToken)
// 	expression := p.parseExpressionAllowIn()
...
// func (p *ZigParser) parseArrayLiteralExpression() *ast.Expression {
// 	pos := p.nodePos()
// 	openBracketPosition := p.scanner.TokenStart()
// 	openBracketParsed := p.parseExpected(ast.KindOpenBracketToken)
// 	multiLine := p.hasPrecedingLineBreak()
// 	elements := p.parseDelimitedList(PCArrayLiteralMembers, (*ZigParser).parseArgumentOrArrayLiteralElement)
...
// func (p *ZigParser) parseObjectLiteralExpression() *ast.Expression {
// 	pos := p.nodePos()
// 	openBracePosition := p.scanner.TokenStart()
// 	openBraceParsed := p.parseExpected(ast.KindOpenBraceToken)
// 	multiLine := p.hasPrecedingLineBreak()
// 	properties := p.parseDelimitedList(PCObjectLiteralMembers, (*ZigParser).parseObjectLiteralElement)
// 	p.parseExpectedMatchingBrackets(ast.KindOpenBraceToken, ast.KindCloseBraceToken, openBraceParsed, openBracePosition)
// 	return p.finishNode(p.factory.NewObjectLiteralExpression(properties, multiLine), pos)
// }

// func (p *ZigParser) parseObjectLiteralElement() *ast.Node {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
// 	if p.parseOptional(ast.KindDotDotDotToken) {
// 		expression := p.parseAssignmentExpressionOrHigher()
...
// 		var initializer *ast.Expression
// 		if equalsToken != nil {
// 			initializer = p.doInContext(ast.NodeFlagsDisallowInContext, false, (*ZigParser).parseAssignmentExpressionOrHigher)
// 		}
...
// func (p *ZigParser) parseFunctionExpression() *ast.Expression {
// 	// GeneratorExpression:
...
// 		name = p.doInContext(ast.NodeFlagsYieldContext|ast.NodeFlagsAwaitContext, true, (*ZigParser).parseOptionalBindingIdentifier)
// 	case isGenerator:
// 		name = p.doInContext(ast.NodeFlagsYieldContext, true, (*ZigParser).parseOptionalBindingIdentifier)
// 	case isAsync:
// 		name = p.doInContext(ast.NodeFlagsAwaitContext, true, (*ZigParser).parseOptionalBindingIdentifier)
// 	default:
...
// func (p *ZigParser) parseOptionalBindingIdentifier() *ast.Node {
...
// func (p *ZigParser) parseDecoratedExpression() *ast.Expression {
// 	pos := p.nodePos()
// 	jsdoc := p.jsdocScannerInfo()
// 	modifiers := p.parseModifiersEx(true /*allowDecorators*/, false /*permitConstAsModifier*/, false /*stopOnStartOfClassStaticBlock*/)
...
// func (p *ZigParser) unparseExpressionWithTypeArguments(expression *ast.Node, typeArguments *ast.NodeList, result *ast.Node) {
...
// func (p *ZigParser) parseNewExpressionOrNewDotTarget() *ast.Node {
// 	pos := p.nodePos()
// 	p.parseExpected(ast.KindNewKeyword)
// 	if p.parseOptional(ast.KindDotToken) {
// 		name := p.parseIdentifierName()
...
// func (p *ZigParser) parseKeywordExpression() *ast.Node {
// 	pos := p.nodePos()
...
// func (p *ZigParser) parseLiteralExpression() *ast.Node {
// 	pos := p.nodePos()
// 	text := p.scanner.TokenValue()
...
// func (p *ZigParser) parseIdentifierNameErrorOnUnicodeEscapeSequence() *ast.Node {
...
// func (p *ZigParser) parseBindingIdentifier() *ast.Node {
// 	return p.parseBindingIdentifierWithDiagnostic(nil)
// }

// func (p *ZigParser) parseBindingIdentifierWithDiagnostic(privateIdentifierDiagnosticMessage *diagnostics.Message) *ast.Node {
// 	saveHasAwaitIdentifier := p.statementHasAwaitIdentifier
...
// func (p *ZigParser) parseIdentifierName() *ast.Node {
// 	return p.parseIdentifierNameWithDiagnostic(nil)
// }

// func (p *ZigParser) parseIdentifierNameWithDiagnostic(diagnosticMessage *diagnostics.Message) *ast.Node {
// 	return p.createIdentifierWithDiagnostic(tokenIsIdentifierOrKeyword(p.token), diagnosticMessage, nil)
// }

// func (p *ZigParser) parseIdentifier() *ast.Node {
// 	return p.parseIdentifierWithDiagnostic(nil, nil)
// }

// func (p *ZigParser) parseIdentifierWithDiagnostic(diagnosticMessage *diagnostics.Message, privateIdentifierDiagnosticMessage *diagnostics.Message) *ast.Node {
// 	return p.createIdentifierWithDiagnostic(p.isIdentifier(), diagnosticMessage, privateIdentifierDiagnosticMessage)
// }

// func (p *ZigParser) createIdentifier(isIdentifier bool) *ast.Node {
// 	return p.createIdentifierWithDiagnostic(isIdentifier, nil, nil)
// }

// func (p *ZigParser) createIdentifierWithDiagnostic(isIdentifier bool, diagnosticMessage *diagnostics.Message, privateIdentifierDiagnosticMessage *diagnostics.Message) *ast.Node {
// 	if isIdentifier {
// 		var pos int
...
// func (p *ZigParser) newNodeList(loc core.TextRange, nodes []*ast.Node) *ast.NodeList {
// 	list := p.factory.NewNodeList(nodes)
// 	list.Loc = loc
// 	return list
// }

// func (p *ZigParser) newModifierList(loc core.TextRange, nodes []*ast.Node) *ast.ModifierList {
// 	list := p.factory.NewModifierList(nodes)
// 	list.Loc = loc
// 	return list
// }

// func (p *ZigParser) finishNode(node *ast.Node, pos int) *ast.Node {
// 	return p.finishNodeWithEnd(node, pos, p.nodePos())
// }

// func (p *ZigParser) finishNodeWithEnd(node *ast.Node, pos int, end int) *ast.Node {
...
// func (p *ZigParser) overrideParentInImmediateChildren(node *ast.Node) {
// 	p.currentParent = node
// 	node.ForEachChild(p.setParentFromContext)
// 	p.currentParent = nil
// }

// func (p *ZigParser) nextTokenIsSlash() bool {
// 	return p.nextToken() == ast.KindSlashToken
// }

// func (p *ZigParser) scanTypeMemberStart() bool {
...
// func (p *ZigParser) scanClassMemberStart() bool {
...
// func (p *ZigParser) canParseSemicolon() bool {
...
// func (p *ZigParser) tryParseSemicolon() bool {
// 	if !p.canParseSemicolon() {
// 		return false
// 	}
// 	if p.token == ast.KindSemicolonToken {
...
// func (p *ZigParser) parseSemicolon() bool {
// 	return p.tryParseSemicolon() || p.parseExpected(ast.KindSemicolonToken)
// }

// func (p *ZigParser) isLiteralPropertyName() bool {
...
// func (p *ZigParser) isStartOfStatement() bool {
// 	switch p.token {
...
// 		return p.isStartOfDeclaration() || !p.lookAhead((*ZigParser).nextTokenIsIdentifierOrKeywordOnSameLine)
...
// func (p *ZigParser) isStartOfDeclaration() bool {
// 	return p.lookAhead((*ZigParser).scanStartOfDeclaration)
// }

// func (p *ZigParser) scanStartOfDeclaration() bool {
// 	for {
// 		switch p.token {
...
// func (p *ZigParser) isStartOfExpression() bool {
...
// func (p *ZigParser) isStartOfLeftHandSideExpression() bool {
// 	switch p.token {
// 	case ast.KindThisKeyword, ast.KindSuperKeyword, ast.KindNullKeyword, ast.KindTrueKeyword, ast.KindFalseKeyword,
...
// func (p *ZigParser) isStartOfType(inStartOfParameter bool) bool {
// 	switch p.token {
// 	case ast.KindAnyKeyword, ast.KindUnknownKeyword, ast.KindStringKeyword, ast.KindNumberKeyword, ast.KindBigIntKeyword,
...
// 	case ast.KindMinusToken:
// 		return !inStartOfParameter && p.lookAhead((*ZigParser).nextTokenIsNumericOrBigIntLiteral)
// 	case ast.KindOpenParenToken:
// 		// Only consider '(' the start of a type if followed by ')', '...', an identifier, a modifier,
// 		// or something that starts a type. We don't want to consider things like '(1)' a type.
// 		return !inStartOfParameter && p.lookAhead((*ZigParser).nextIsParenthesizedOrFunctionType)
// 	}
// 	return p.isIdentifier()
// }

// func (p *ZigParser) nextTokenIsNumericOrBigIntLiteral() bool {
// 	p.nextToken()
// 	return p.token == ast.KindNumericLiteral || p.token == ast.KindBigIntLiteral
// }

// func (p *ZigParser) nextIsParenthesizedOrFunctionType() bool {
// 	p.nextToken()
...
// func (p *ZigParser) isStartOfParameter(isJSDocParameter bool) bool {
...
// func (p *ZigParser) isBindingIdentifierOrPrivateIdentifierOrPattern() bool {
...
// func (p *ZigParser) isNextTokenOpenParenOrLessThanOrDot() bool {
// 	return p.lookAhead((*ZigParser).nextTokenIsOpenParenOrLessThanOrDot)
// }

// func (p *ZigParser) nextTokenIsOpenParenOrLessThanOrDot() bool {
// 	switch p.nextToken() {
...
// func (p *ZigParser) nextTokenIsIdentifierOnSameLine() bool {
// 	p.nextToken()
// 	return p.isIdentifier() && !p.hasPrecedingLineBreak()
// }

// func (p *ZigParser) nextTokenIsIdentifierOrStringLiteralOnSameLine() bool {
// 	p.nextToken()
// 	return (p.isIdentifier() || p.token == ast.KindStringLiteral) && !p.hasPrecedingLineBreak()
// }

// // Ignore strict mode flag because we will report an error in type checker instead.
// func (p *ZigParser) isIdentifier() bool {
...
// func (p *ZigParser) isBindingIdentifier() bool {
// 	// `let await`/`let yield` in [Yield] or [Await] are allowed here and disallowed in the binder.
// 	return p.token == ast.KindIdentifier || p.token > ast.KindLastReservedWord
// }

// func (p *ZigParser) isImportAttributeName() bool {
// 	return tokenIsIdentifierOrKeyword(p.token) || p.token == ast.KindStringLiteral
// }

// func (p *ZigParser) isBinaryOperator() bool {
...
// func (p *ZigParser) isValidHeritageClauseObjectLiteral() bool {
// 	return p.lookAhead((*ZigParser).nextIsValidHeritageClauseObjectLiteral)
// }

// func (p *ZigParser) nextIsValidHeritageClauseObjectLiteral() bool {
...
// func (p *ZigParser) isHeritageClause() bool {
// 	return p.token == ast.KindExtendsKeyword || p.token == ast.KindImplementsKeyword
// }

// func (p *ZigParser) isHeritageClauseExtendsOrImplementsKeyword() bool {
// 	return p.isHeritageClause() && p.lookAhead((*ZigParser).nextIsStartOfExpression)
// }

// func (p *ZigParser) nextIsStartOfExpression() bool {
// 	p.nextToken()
// 	return p.isStartOfExpression()
// }

// func (p *ZigParser) isUsingDeclaration() bool {
...
// 	return p.lookAhead(func(p *ZigParser) bool {
// 		return p.nextTokenIsBindingIdentifierOrStartOfDestructuringOnSameLine( /*disallowOf*/ false)
// 	})
// }

// func (p *ZigParser) nextTokenIsEqualsOrSemicolonOrColonToken() bool {
// 	p.nextToken()
// 	return p.token == ast.KindEqualsToken || p.token == ast.KindSemicolonToken || p.token == ast.KindColonToken
// }

// func (p *ZigParser) nextTokenIsBindingIdentifierOrStartOfDestructuringOnSameLine(disallowOf bool) bool {
// 	p.nextToken()
// 	if disallowOf && p.token == ast.KindOfKeyword {
// 		return p.lookAhead((*ZigParser).nextTokenIsEqualsOrSemicolonOrColonToken)
// 	}
// 	return (p.isBindingIdentifier() || p.token == ast.KindOpenBraceToken) && !p.hasPrecedingLineBreak()
// }

// func (p *ZigParser) nextTokenIsBindingIdentifierOrStartOfDestructuringOnSameLineDisallowOf() bool {
// 	return p.nextTokenIsBindingIdentifierOrStartOfDestructuringOnSameLine( /*disallowOf*/ true)
// }

// func (p *ZigParser) isAwaitUsingDeclaration() bool {
// 	return p.lookAhead((*ZigParser).nextIsUsingKeywordThenBindingIdentifierOrStartOfObjectDestructuringOnSameLine)
// }

// func (p *ZigParser) nextIsUsingKeywordThenBindingIdentifierOrStartOfObjectDestructuringOnSameLine() bool {
// 	return p.nextToken() == ast.KindUsingKeyword && p.nextTokenIsBindingIdentifierOrStartOfDestructuringOnSameLine( /*disallowOf*/ false)
// }

// func (p *ZigParser) nextTokenIsTokenStringLiteral() bool {
// 	return p.nextToken() == ast.KindStringLiteral
// }

// func (p *ZigParser) setContextFlags(flags ast.NodeFlags, value bool) {
// 	if value {
...
// func (p *ZigParser) doInContext[T any](flags ast.NodeFlags, value bool, f func(p *ZigParser) T) T {
// 	saveContextFlags := p.contextFlags
...
// func (p *ZigParser) inYieldContext() bool {
// 	return p.contextFlags&ast.NodeFlagsYieldContext != 0
// }

// func (p *ZigParser) inDisallowInContext() bool {
// 	return p.contextFlags&ast.NodeFlagsDisallowInContext != 0
// }

// func (p *ZigParser) inDisallowConditionalTypesContext() bool {
// 	return p.contextFlags&ast.NodeFlagsDisallowConditionalTypesContext != 0
// }

// func (p *ZigParser) inDecoratorContext() bool {
// 	return p.contextFlags&ast.NodeFlagsDecoratorContext != 0
// }

// func (p *ZigParser) inAwaitContext() bool {
// 	return p.contextFlags&ast.NodeFlagsAwaitContext != 0
// }

// func (p *ZigParser) skipRangeTrivia(textRange core.TextRange) core.TextRange {
...
// func (p *ZigParser) processPragmasIntoFields(context *ast.SourceFile) {
...
// func (p *ZigParser) parseResolutionMode(mode string, pos int, end int) (resolutionKind core.ResolutionMode) {
// 	if mode == "import" {
...
// func (p *ZigParser) jsErrorAtRange(loc core.TextRange, message *diagnostics.Message, args ...any) {
...
// func (p *ZigParser) checkJSDecoratorSyntax(node *ast.Node) {
...
// func (p *ZigParser) checkJSSyntax(node *ast.Node) *ast.Node {
